...
export default function ParentsTaxPage() {
  ...
  async function load() {
    const res = await fetch("/api/tax/parents");
    ...
    <div className="min-h-screen bg-zinc-50 px-4 py-8 max-w-md mx-auto">
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 p-5 mb-4">
        ...
        <div className="flex items-end justify-between">
          ...
          <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${
            data.nextDue.daysAway <= 14 ? "bg-red-100 text-red-600"
            : data.nextDue.daysAway <= 30 ? "bg-amber-100 text-amber-600"
      ...
      <div className="bg-white rounded-2xl border border-zinc-200 p-5 mb-4">
        ...
        <div className="flex gap-2">
          <input type="number" step="0.01" min="0" value={setAsideInput}
  ...
  return (
    <div className="flex justify-between items-center py-0.5">
      ...
      <span className={`text-sm ${
        highlight ? "text-amber-600 font-semibold" : bold ? "text-zinc-800 font-semibold" : sub ? "text-zinc-400" : "text-zinc-700"
      }`}>{value}</span>
    </div>
  );
}
