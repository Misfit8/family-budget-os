...
// Household user IDs for gig parents
const PARENT_USERS = [
  { id: 1, name: "Parent 1" },
  { id: 2, name: "Parent 2" },
];
...
export default function AccountsPage() {
  ...
  const [accounts, setAccounts] = useState<Record<number, { accounts: TellerAccount[]; enrollments: TellerEnrollment[] }>>({});
  ...
  useEffect(() => { loadTransactions(selectedMonth); }, [selectedMonth, loadTransactions]);
  ...
  async function sync(userId: number) {
    ...
    await Promise.all([loadAccounts(), loadTransactions(selectedMonth)]);
  }
  ...
  const combinedTotalIn = PARENT_USERS.reduce((s, u) => s + (txnData[u.id]?.totalIn ?? 0), 0);
  const combinedTotalOut = PARENT_USERS.reduce((s, u) => s + (txnData[u.id]?.totalOut ?? 0), 0);
  const net = combinedTotalIn - combinedTotalOut;

  // Merge categories across both parents
  const allCategories: Record<string, number> = {};
  for (const u of PARENT_USERS) {
  ...
  const activeData = txnData[activeUser];
  const activeAccountData = accounts[activeUser];
    ...
    <div className="min-h-screen bg-zinc-50 px-4 py-8 max-w-md mx-auto">
      ...
      <div className="mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          {(activeAccountData?.accounts?.length ?? 0) > 0 && (
            <button
              onClick={() => sync(activeUser)}
              disabled={syncing[activeUser]}
              className="text-xs text-zinc-500 border border-zinc-200 rounded-lg px-3 py-1.5 hover:border-zinc-400 disabled:opacity-50"
            >
              {syncing[activeUser] ? "Syncing…" : "Sync Now"}
            </button>
          )}
        </div>
        ...
        {(activeAccountData?.accounts?.length ?? 0) === 0 ? (
          ...
          <div className="flex flex-col gap-2 mb-3">
            {activeAccountData.accounts.map((acct) => (
            ...
            {/* Unlink enrolled institutions */}
            {activeAccountData.enrollments.map((enr) => (
              <button
                key={enr.enrollment_id}
                onClick={() => unlink(activeUser, enr.enrollment_id)}
        ...
        <TellerConnect
          userId={activeUser}
          label={`Link ${PARENT_USERS.find((u) => u.id === activeUser)?.name} Bank Account`}
          onConnected={() => { loadAccounts(); sync(activeUser); }}
        />
      </div>
      ...
      {(activeData?.transactions?.length ?? 0) > 0 && (
        <div className="mt-4">
          <p className="text-xs text-zinc-400 uppercase tracking-widest mb-3">
            Transactions — {PARENT_USERS.find((u) => u.id === activeUser)?.name}
          </p>
